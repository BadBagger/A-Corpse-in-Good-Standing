...
def grade_pixel(r: int, g: int, b: int, a: int) -> tuple[int, int, int, int]:
    ...
    if 70.0 <= hue <= 170.0 and s > 0.08:
        ...
        g = clamp((g * (1.0 - blend)) + (target[1] * blend) - 10)
...
def main() -> None:
    ...
    report = {
        ...
        "palette_rule": "Act I character overlays keep green reserved for wrong light; standee and crowd shadows are biased toward harbor slate and whale-oil amber.",
        "records": records,
    ...
    if records:
        ...
        for record in records:
            lines.append(
                ...
                f"{record['wrong_light_green_percent_after']}% | {record['wrong_light_green_delta']}% |"
            )
    lines.extend(
        [
            "",
    ...
    for record in analysis_records:
        lines.append(f"| `{record['asset']}` | {record['wrong_light_green_percent']}% |")
