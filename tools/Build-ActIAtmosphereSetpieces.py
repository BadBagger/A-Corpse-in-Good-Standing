...
ATMOSPHERE = [
    {
        "id": "mudflats_tide_glint",
        "room_code": "R01",
        "room_folder": "mudflats",
        "background": "game/rooms/mudflats/background/mudflats_bg.png",
        "x": 0,
        "y": 610,
        "width": 1920,
        "height": 360,
...
def make_tide_glint(region: Image.Image, index: int, frames: int) -> Image.Image:
    phase = (index / frames) * math.tau
    crop = region.filter(ImageFilter.GaussianBlur(2.2))
    crop = ImageEnhance.Contrast(crop).enhance(1.02)
    crop = ImageEnhance.Brightness(crop).enhance(0.56 + 0.018 * math.sin(phase))
    overlay = Image.new("RGBA", region.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay, "RGBA")
    for band in range(9):
        y = 34 + band * 31
        wobble = int(math.sin(phase + band * 0.72) * 34)
        draw.line((120 + wobble, y, region.width - 240 + wobble, y + 5), fill=(*BONE, 5), width=1)
        draw.line((40 - wobble, y + 14, region.width // 2 - wobble, y + 19), fill=(*HARBOR_SLATE, 8), width=1)
    for band in range(4):
        y = 170 + band * 34
        wobble = int(math.sin(phase * 1.4 + band) * 22)
        draw.line((region.width // 2 + wobble, y, region.width - 120 + wobble, y + 7), fill=(*AMBER, 4), width=1)
    frame = Image.alpha_composite(crop, overlay)
    frame.putalpha(transparent_blur_mask(region.size, 84))
    return frame
...
def make_frame(region: Image.Image, item: dict[str, object], index: int) -> Image.Image:
    ...
    if kind == "tide_glint":
        return make_tide_glint(region, index, frames)
    if kind == "water_glint":
...
def main() -> None:
    ...
    lines = [
        ...
        "",
        "These are subtle transparent runtime overlays: tide glint, water glint, lamp flicker, smoke, steam, and window rain. They add motion without changing puzzle coordinates or replacing final paintover sources.",
        "",
