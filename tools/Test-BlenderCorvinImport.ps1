...
bpy.ops.wm.save_as_mainfile(filepath=character_blend)
bpy.ops.wm.save_as_mainfile(filepath=shader_blend)
...
result = {
    ...
    "saved_character_blend": os.path.exists(character_blend),
    "saved_shader_blend": os.path.exists(shader_blend)
}
...
"@
    ...
    elseif (-not (Test-Path -LiteralPath $blenderResultJson)) {
        ...
        elseif (-not $result.saved_character_blend -or -not $result.saved_shader_blend) {
            $status = "failed"
            $notes.Add("Blender imported the GLB but did not save both starter blend files.")
        }
        else {
            $status = "audited"
            $notes.Add("Blender imported Corvin Act I clean GLB and saved starter blend files.")
