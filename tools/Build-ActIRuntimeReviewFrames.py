...
def add_hud(image: Image.Image, room_code: str, title: str) -> None:
    ...
    status = status.resize((round(status.width * 1.0), round(status.height * 0.54)), Image.Resampling.LANCZOS)
    dialogue = dialogue.resize((round(dialogue.width * 0.67), round(dialogue.height * 0.62)), Image.Resampling.LANCZOS)
    inventory = inventory.resize((round(inventory.width * 0.68), round(inventory.height * 0.46)), Image.Resampling.LANCZOS)

    image.alpha_composite(status, (12, 44))
    image.alpha_composite(dialogue, (710, 860))
    image.alpha_composite(inventory, (24, 898))

    draw = ImageDraw.Draw(image, "RGBA")
    font = ImageFont.load_default()
    draw.rectangle((18, 18, 430, 76), fill=(12, 16, 19, 184), outline=(201, 138, 60, 120))
    draw.text((34, 34), f"{room_code} / {title}", fill=PALETTE["bone"], font=font)
    draw.text((34, 52), "LOOK  USE  TALK  WET    Inventory: empty", fill=PALETTE["amber"], font=font)
    draw.text((744, 900), "Corvin: dead, damp, and still doing the voice.", fill=PALETTE["bone"], font=font)
