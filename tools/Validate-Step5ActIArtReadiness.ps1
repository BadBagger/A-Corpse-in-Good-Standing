...
$dialoguePortraitHudValidatorPath = Join-Path $root "tools\Validate-ActIDialoguePortraitHud.ps1"
$livingSceneCuesValidatorPath = Join-Path $root "tools\Validate-ActILivingSceneCues.ps1"
...
$gameplayReviewPanelsValidatorPath = Join-Path $root "tools\Validate-ActIGameplayReviewPanels.ps1"
...
$inSceneActionReviewPath = Join-Path $root "docs\art\act_i_in_scene_action_review.md"
$inSceneActionReviewJsonPath = Join-Path $root "docs\art\act_i_in_scene_action_review.json"
$inSceneActionReviewImagePath = Join-Path $root "docs\art\review\act_i_in_scene_action_contact_sheet.png"
$inSceneActionReviewValidatorPath = Join-Path $root "tools\Validate-ActIInSceneActionReview.ps1"
...
$characterOcclusionValidatorPath = Join-Path $root "tools\Validate-ActICharacterOcclusion.ps1"
$renderedCharacterGreenFitValidatorPath = Join-Path $root "tools\Validate-ActIRenderedCharacterGreenFit.ps1"
...
$playableHoverFocusValidatorPath = Join-Path $root "tools\Validate-ActIPlayableHoverFocus.ps1"
...
foreach ($path in @(
    ...
    $dialoguePortraitHudValidatorPath,
    $livingSceneCuesValidatorPath,
    ...
    $gameplayReviewPanelsValidatorPath,
    ...
    $inSceneActionReviewPath,
    $inSceneActionReviewJsonPath,
    $inSceneActionReviewImagePath,
    $inSceneActionReviewValidatorPath,
    $characterPaletteGradePath,
    $characterPaletteGradeJsonPath,
    $characterPaletteGradeValidatorPath,
    $characterSceneFitValidatorPath,
    $characterOcclusionValidatorPath,
    $renderedCharacterGreenFitValidatorPath,
    ...
    $playableHoverFocusValidatorPath
)) {
...
$gameplayReviewPanels = Get-Content -LiteralPath $gameplayReviewPanelsPath -Raw
$gameplayReviewPanelsJson = Get-Content -LiteralPath $gameplayReviewPanelsJsonPath -Raw | ConvertFrom-Json
...
$inSceneActionReview = Get-Content -LiteralPath $inSceneActionReviewPath -Raw
...
& powershell -NoProfile -ExecutionPolicy Bypass -File $dialoguePortraitHudValidatorPath
if ($LASTEXITCODE -ne 0) {
    throw "Act I dialogue portrait HUD validator failed."
}

& powershell -NoProfile -ExecutionPolicy Bypass -File $livingSceneCuesValidatorPath
if ($LASTEXITCODE -ne 0) {
    throw "Act I living scene cue validator failed."
}

& powershell -NoProfile -ExecutionPolicy Bypass -File $corvinRuntimeReadabilityRimValidatorPath
if ($LASTEXITCODE -ne 0) {
...
foreach ($requiredText in @(
    ...
    "larger crops",
    "room-specific dialogue captions",
    "Crop box"
)) {
    if ($gameplayReviewPanels -notmatch [regex]::Escape($requiredText)) {
...
& powershell -NoProfile -ExecutionPolicy Bypass -File $inSceneActionReviewValidatorPath
if ($LASTEXITCODE -ne 0) {
    throw "Act I in-scene action review validator failed."
}

if ($inSceneActionReviewJson.status -ne "exported" -or [int]$inSceneActionReviewJson.frame_count -ne 9) {
    throw "Act I in-scene action review must export 9 action-in-room frames."
}
foreach ($requiredText in @(
    ...
    "turn_to_corvin",
    "named NPC cases",
    "text-free interaction effects",
    "static idle room shots"
)) {
    if ($inSceneActionReview -notmatch [regex]::Escape($requiredText)) {
        throw "Act I in-scene action review report missing required text: $requiredText"
    }
}
...
& powershell -NoProfile -ExecutionPolicy Bypass -File $characterOcclusionValidatorPath
if ($LASTEXITCODE -ne 0) {
    throw "Act I character occlusion validator failed."
}

& powershell -NoProfile -ExecutionPolicy Bypass -File $renderedCharacterGreenFitValidatorPath
if ($LASTEXITCODE -ne 0) {
    throw "Act I rendered character green-fit validator failed."
}
...
& powershell -NoProfile -ExecutionPolicy Bypass -File $playableHoverFocusValidatorPath
if ($LASTEXITCODE -ne 0) {
    throw "Act I playable hover focus validator failed."
}
...
$gameplayReviewPanelCount = [int]$gameplayReviewPanelsJson.panel_count
...
$lines = @(
    ...
    "- Act I Godot runtime frames: pass, $godotRuntimeFrameCount runtime-composition player-view frames show actual room scene background paths, shared art constants, foreground props, wet-floor reflections, HUD textures, looping living scene cues, speaker portraits embedded in the dialogue HUD, NPC standees, in-world hotspot glints, Corvin side sprites, Corvin's subtle amber readability rim, standee character integration rims, and foreground character occluders.",
    "- Act I dialogue portrait HUD: pass, generated speaker portraits are embedded in the playable HUD and runtime review frames for Corvin, Tomas, Registrar, Prosper, Juno, and Sabine.",
    ...
    "- Act I in-scene action review: pass, $inSceneActionFrameCount room-composition frames show Corvin talk/use/wet actions in context, including the Salt Market crowd turn_to_corvin state, named NPC dialogue/counter staging, text-free action focus marks, text-free wet interaction effects, and zoom crops for action readability.",
    ...
    "- Act I rendered character green-fit: pass, final runtime frame character zones keep green dominance below the scene-fit thresholds while Church wrong-light remains environment-only.",
...
foreach ($requiredText in @(
    ...
    "Act I dialogue portrait HUD: pass",
    "Act I living scene cues: pass",
    "in-world hotspot glints",
    "Act I in-scene action review: pass",
    "Act I character palette grade: pass",
    "Act I character scene fit: pass",
    "Act I character occlusion: pass",
    "Act I rendered character green-fit: pass",
    ...
    "Act I playable hover focus: pass",
