...
SOURCE = Path(
    r"C:\Users\KyleB\.codex\generated_images\019fed98-2fe1-7052-afd1-585c9b506c3f\call_WWUFfCTjsm18U3naXqzfn7El.png"
)
...
PROPS = [
    {
        "id": "calcified_bollard_row",
        "name": "Calcified bollard row",
    ...
    {
        "id": "salt_rope_cleat",
        "name": "Salt rope cleat",
        "crop": (0.50, 0.02, 1.00, 0.50),
        "target_height": 155,
        "position": (655, 740),
        "z": 5,
    },
    {
        "id": "empty_flask",
        "name": "Empty flask",
        "crop": (0.00, 0.55, 0.48, 1.00),
        "target_height": 105,
        "position": (1115, 735),
        "z": 5,
    },
    {
        "id": "quay_crate_cluster",
        "name": "Quay crate cluster",
        "crop": (0.48, 0.50, 1.00, 1.00),
        "target_height": 185,
        "position": (1365, 700),
        "z": 4,
    },
...
def main() -> None:
    if not SOURCE.exists():
        raise FileNotFoundError(f"Missing OpenAI Old Quay prop sheet: {SOURCE}")

    src_dir = ROOT / "art" / "src" / "props" / "old_quay" / "openai"
    export_dir = ROOT / "art" / "export" / "props" / "old_quay"
    game_dir = ROOT / "game" / "rooms" / "old_quay" / "props"
    review_dir = ROOT / "docs" / "art" / "review"
    for directory in (src_dir, export_dir, game_dir, review_dir):
        directory.mkdir(parents=True, exist_ok=True)

    source_copy = src_dir / "old_quay_props_openai_raw.png"
    ...
    contact_path = review_dir / "old_quay_openai_props_contact_sheet.png"
    make_contact_sheet(manifest).save(contact_path)

    json_path = ROOT / "docs" / "art" / "old_quay_openai_props.json"
    json_path.write_text(json.dumps({"props": manifest}, indent=2) + "\n", encoding="utf-8")

    md_lines = [
        "# Old Quay OpenAI Foreground Props",
        "",
        "Act I Old Quay foreground prop cutouts extracted from an OpenAI-generated source sheet, alpha-preserved, palette-locked, and staged for runtime use.",
        "",
        ...
        "- Palette note: foreground props avoid absinthe green and arterial red; the quay stays slate, salt, amber, and wet black.",
        "",
    ...
    md_lines.append("")
    (ROOT / "docs" / "art" / "old_quay_openai_props.md").write_text("\n".join(md_lines), encoding="utf-8")

    print(f"Imported Old Quay OpenAI props: count={len(manifest)}, contact={contact_path}")
