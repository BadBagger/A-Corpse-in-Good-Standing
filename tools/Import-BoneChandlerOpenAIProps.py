...
SOURCE = Path(
    r"C:\Users\KyleB\.codex\generated_images\019fed98-2fe1-7052-afd1-585c9b506c3f\call_5CPysoRoDEX00pSXzq9nXZme.png"
)
...
PROPS = [
    {
        "id": "prosper_watch_display",
        "name": "Prosper watch display",
        "cell": (0, 0),
        "target_height": 155,
        "position": (1018, 612),
        "z": 5,
        "hotspot": "ProsperWatch",
    },
    {
        "id": "bone_trade_counter",
        "name": "Bone trade counter",
        "cell": (1, 0),
        "target_height": 215,
        "position": (690, 545),
        "z": 3,
    ...
    {
        "id": "bone_shelf_cluster",
        "name": "Bone shelf collateral",
        "cell": (1, 1),
        "target_height": 205,
        "position": (1325, 500),
        "z": 3,
...
def palette_lock(image: Image.Image) -> Image.Image:
    ...
    for y in range(rgba.height):
        for x in range(rgba.width):
            ...
            # Bone Chandler props should read bone/amber/slate. Green is not a
            # room cue here, and arterial red is reserved for rare story beats.
...
def main() -> None:
    if not SOURCE.exists():
        raise FileNotFoundError(f"Missing OpenAI Bone Chandler prop sheet: {SOURCE}")

    src_dir = ROOT / "art" / "src" / "props" / "bone_chandler" / "openai"
    export_dir = ROOT / "art" / "export" / "props" / "bone_chandler"
    game_dir = ROOT / "game" / "rooms" / "bone_chandler" / "props"
    review_dir = ROOT / "docs" / "art" / "review"
    for directory in (src_dir, export_dir, game_dir, review_dir):
        directory.mkdir(parents=True, exist_ok=True)

    source_copy = src_dir / "bone_chandler_props_openai_raw.png"
    ...
    contact_path = review_dir / "bone_chandler_openai_props_contact_sheet.png"
    make_contact_sheet(manifest).save(contact_path)

    json_path = ROOT / "docs" / "art" / "bone_chandler_openai_props.json"
    json_path.write_text(json.dumps({"props": manifest}, indent=2) + "\n", encoding="utf-8")

    md_lines = [
        "# Bone Chandler OpenAI Foreground Props",
        "",
        "Foreground prop cutouts extracted from an OpenAI-generated Bone Chandler source sheet, alpha-preserved, palette-locked, and staged for runtime use.",
        "",
    ...
    md_lines.append("")
    (ROOT / "docs" / "art" / "bone_chandler_openai_props.md").write_text(
        "\n".join(md_lines), encoding="utf-8"
    )

    print(f"Imported Bone Chandler OpenAI props: count={len(manifest)}, contact={contact_path}")
