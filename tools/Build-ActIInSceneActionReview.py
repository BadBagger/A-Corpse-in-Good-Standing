...
CASES = [
    ...
    },
    {
        ...
        "pulse": {"position": (655, 740), "radius": 82, "color": (228, 220, 200, 148), "label": "wet rope darkens"},
    ...
    },
    {
        ...
        "pulse": {"position": (890, 650), "radius": 108, "color": (125, 155, 78, 154), "label": "lamp smokes"},
    ...
    },
    {
        ...
        "standees": [],
        "setpieces": [],
        "pulse": {"position": (1410, 725), "radius": 96, "color": (228, 220, 200, 138), "label": "water finds the grate"},
...
def draw_interaction_pulse(image: Image.Image, pulse: dict[str, object]) -> dict[str, object]:
    ...
    draw.text((x - 80, y - 72), label, fill=(228, 220, 200, 220))
    return {"position": [int(x), int(y)], "radius": radius, "label": label}
...
def main() -> None:
    ...
    report = {
        ...
        "runtime_evidence": "In-scene action review frames show Corvin talk/use/wet side actions inside Act I rooms, including the Salt Market crowd turn_to_corvin sectional setpiece state, named NPC standee dialogue/counter staging, and visible wet interaction pulses.",
        "frames": records,
    }
    ...
    lines = [
        ...
        "",
        "These review frames show Corvin's talk, use, and wet side actions inside Act I room compositions. The Salt Market cases use the crowd `turn_to_corvin` sectional setpiece state, the named NPC cases include standees, contact shadows, and wet-floor reflections, and the wet cases include visible interaction pulses, so the review proof covers interaction staging instead of only static idle room shots.",
        "",
    ...
    for record in records:
        ...
        pulse_label = pulse.get("label", "-") if pulse else "-"
        lines.append(f"| {record['id']} | {record['room_code']} / {record['title']} | `{record['corvin_animation']}` | {record['standee_count']} | {pulse_label} | `{record['expected_setpiece_state']}` | `{record['output']}` |")
