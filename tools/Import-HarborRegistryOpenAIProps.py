...
SOURCE = Path(
    r"C:\Users\KyleB\.codex\generated_images\019fed98-2fe1-7052-afd1-585c9b506c3f\call_v9lnDh8zUH43xYZu4O1o1aQu.png"
)
...
PROPS = [
    {
        "id": "registry_roll_book",
        "name": "Open harbor roll book",
        "cell": (0, 0),
        "target_height": 190,
        "position": (520, 555),
        "z": 3,
    ...
    {
        "id": "registry_confession_slips",
        "name": "Confession slips and wax seals",
        "cell": (1, 0),
        "target_height": 118,
        "position": (1065, 720),
        "z": 4,
        "hotspot": "KestrelLedger",
    },
    {
        "id": "registry_inkstand",
        "name": "Inkstand and notary stamp",
        "cell": (0, 1),
        "target_height": 110,
        "position": (790, 720),
        "z": 5,
    ...
    {
        "id": "registry_candles",
        "name": "Candles and brass nameplate",
        "cell": (1, 1),
        "target_height": 118,
        "position": (910, 642),
        "z": 4,
...
def palette_lock(image: Image.Image) -> Image.Image:
    ...
    for y in range(rgba.height):
        for x in range(rgba.width):
            ...
            # Keep registry props amber/bone/slate. Green belongs to the lamp
            # and stained glass, and arterial red is too scarce to spend on
            # generated wax/stamp clutter.
            if (nr, ng, nb) == (0x7D, 0x9B, 0x4E):
...
def main() -> None:
    if not SOURCE.exists():
        raise FileNotFoundError(f"Missing OpenAI Harbor Registry prop sheet: {SOURCE}")

    src_dir = ROOT / "art" / "src" / "props" / "harbor_registry" / "openai"
    export_dir = ROOT / "art" / "export" / "props" / "harbor_registry"
    game_dir = ROOT / "game" / "rooms" / "harbor_registry" / "props"
    review_dir = ROOT / "docs" / "art" / "review"
    for directory in (src_dir, export_dir, game_dir, review_dir):
        directory.mkdir(parents=True, exist_ok=True)

    source_copy = src_dir / "harbor_registry_props_openai_raw.png"
    ...
    contact_path = review_dir / "harbor_registry_openai_props_contact_sheet.png"
    make_contact_sheet(manifest).save(contact_path)

    json_path = ROOT / "docs" / "art" / "harbor_registry_openai_props.json"
    json_path.write_text(json.dumps({"props": manifest}, indent=2) + "\n", encoding="utf-8")

    md_lines = [
        "# Harbor Registry OpenAI Foreground Props",
        "",
        "Foreground prop cutouts extracted from an OpenAI-generated Harbor Registry source sheet, alpha-preserved, palette-locked, and staged for runtime use.",
        "",
    ...
    md_lines.append("")
    (ROOT / "docs" / "art" / "harbor_registry_openai_props.md").write_text(
        "\n".join(md_lines), encoding="utf-8"
    )

    print(f"Imported Harbor Registry OpenAI props: count={len(manifest)}, contact={contact_path}")
