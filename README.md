...
- `tools\Validate-Step5HumanReviewBundle.ps1` regenerates and validates the compact Step 5 human-review bundle, keeping the launch command, latest notes, decision CSV, contact sheet, hotspot overlay, paintover packet, duel-format lock, and Grey Float hard-R lock together.
...
- `tools\Validate-ActIReviewHandoffSync.ps1` validates that the latest human review notes, decision CSV, review tracker, contact sheet, and Step 5 dashboard all reference the same 11 Act I rooms before approvals are imported.
- `tools\Test-ActIReviewHandoffSync.ps1` proves the review handoff sync gate accepts the current handoff and rejects decision-mismatch, missing-room, and missing-contact-sheet negative controls before restoring artifacts.
